# Radius — PHASE 5: Shipyard & Iterate Mode
# DigitalOcean Hackathon

---

## CONTEXT RECAP
Radius is a multi-agent SaaS incubation platform. Phases 1–4 complete:
...
export const github = {
  ...
  pushDirectory: async (localPath: string, repoUrl: string, branch = 'main') => {
    ...
    execSync(`git init && git add -A && git commit -m "chore: initial Radius scaffold"`, { cwd: localPath })
...
export async function createLocalStack(params: {
  ...
  const outDir = `/tmp/radius/${params.projectId}`
...
function generateLocalReadme(projectId: string): string {
  return `# Radius Generated App

Generated by Radius (project: ${projectId})
...
export async function runShipyard(data: ShipyardJobData): Promise<Record<string, unknown>> {
  ...
  if (!deployment.stepBDone) {
    ...
    if (mode === 'NEW') {
      const repo = await github.createRepo(repoSlug, `Radius generated: ${concept}`)
