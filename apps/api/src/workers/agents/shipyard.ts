...
export async function runShipyard(
  ...
  const localPath = `/tmp/radius/repos/${projectId}`;
  ...
  if (!deployment.stepADone) {
    ...
    await timed(
      ...
      async () => {
        ...
        await fs.promises.writeFile(
          path.join(localPath, "zerops.yaml"),
  ...
  if (!deployment.stepBDone) {
    ...
    if (mode === "NEW") {
      ...
      const safeDescription = `Radius generated: ${sanitizedConcept.slice(0, 150)}${sanitizedConcept.length > 150 ? "..." : ""}`;
  ...
  // ─── STEP C: Prepare Zerops GitHub handoff ────────────────────────────────
  if (!deployment.stepCDone && mode === "NEW") {
    ...
    await timed(
      projectId,
      "Step C: Zerops GitHub handoff ready",
      async () => {
        await log(
          projectId,
          "info",
          `Connect ${actualRepo} to a Zerops runtime service using setup "app".`,
        );
      },
    );
  ...
  // ─── STEP D: Mark package ready for Zerops deploy ─────────────────────────
  if (!deployment.stepDDone && mode === "NEW") {
    ...
    await timed(
      projectId,
      "Step D: Zerops-ready package verification",
      async () => {
        const generatedZeropsYaml = path.join(localPath, "zerops.yaml");
        await fs.promises.access(generatedZeropsYaml);
      },
    );

    await log(projectId, "info", "Step D: Repository is ready for Zerops GitHub deploy");

    deployment = await prisma.deployment.update({
      where: { projectId },
      data: { stepDDone: true, buildStatus: "ACTIVE" },
    });
    ...
    await publishEvent(projectId, {
      ...
      zeropsAppUrl: deployment.zeropsAppUrl ?? "",
...
// After successful deployment: upload project summary to S3-compatible memory storage.
...
Tags: ${[...output.techStack.frontend, ...output.techStack.backend].map((t) => t.toLowerCase().replace(/[^a-z0-9]/g, "-")).join(", ")}`;
    ...
    const accessKeyId = settings.storageAccessKeyId || process.env.STORAGE_ACCESS_KEY_ID;
    const secretAccessKey =
      settings.storageSecretAccessKey || process.env.STORAGE_SECRET_ACCESS_KEY;
    const bucket = settings.storageBucket || process.env.STORAGE_BUCKET;
    if (!accessKeyId || !secretAccessKey || !bucket) {
      console.log("[Shipyard] Skipping memory upload: storage credentials are not configured");
      return;
    }

    const region =
      settings.storageRegion || (process.env.STORAGE_REGION ?? "us-east-1");
