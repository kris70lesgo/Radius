...
async function start() {
  ...
  app.listen(PORT, () => {
    ...
    console.log(`AI_BASE_URL:           ${process.env.AI_BASE_URL ?? 'https://agentrouter.org/v1'}`)
    console.log(`AI_MODEL:              ${process.env.AI_MODEL ?? 'gpt-5.6-sol'}`)
    ...
    console.log(`GOLDEN_BOILERPLATE:    ${process.env.GOLDEN_BOILERPLATE_REPO ?? '❌ MISSING'}`)
    console.log(`ZEROPS_PROJECT_ID:     ${process.env.ZEROPS_PROJECT_ID ?? '❌ MISSING'}`)
