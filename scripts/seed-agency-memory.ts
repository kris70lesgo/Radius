...
const spaces = new S3Client({
    endpoint: process.env.STORAGE_ENDPOINT || `https://${process.env.STORAGE_REGION || 'us-east-1'}.digitaloceanspaces.com`,
    region: process.env.STORAGE_REGION ?? 'us-east-1',
    credentials: {
        accessKeyId: process.env.STORAGE_ACCESS_KEY_ID || process.env.DO_SPACES_KEY!,
        secretAccessKey: process.env.STORAGE_SECRET_ACCESS_KEY || process.env.DO_SPACES_SECRET!,
    },
})

const BUCKET = process.env.STORAGE_BUCKET || process.env.DO_SPACES_BUCKET!
const KB_UUID = process.env.DO_KNOWLEDGE_BASE_UUID || process.env.KB_UUID || ''
const KB_API_TOKEN = process.env.DO_API_TOKEN || process.env.KB_API_TOKEN || ''
